...
parser.add_argument('--dist-url', default='tcp://local:54078', type=str,
...
parser.add_argument('--quan_bit', default=4, type=int,
                    help='quantization bit num.')

"设置w和input不同比特数"
# parser.add_argument('--quan_bit_w', default=8, type=int,
#                     help='quantization bit_w num.')
# parser.add_argument('--quan_bit_input', default=8, type=int,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.quantize is not None:        
        ...
        if args.quantize == "uniform":		    
            ...
            print("Using DSQConv...")
            transformer.register(nn.Conv2d, DSQConv) #改变注册层的名字
    ...
    for module_name in model._modules:
        if len(model._modules[module_name]._modules) > 0:#递归至基础模型
            ...
            if hasattr(model._modules[module_name], "num_bit"):#对基础模型进行赋值
                ...
                #print(model._modules[module_name].num_bit)
    return model
...
def set_quanbit_input(model, quan_bit_input=8,a=0):  # 设置比特数
    ...
    for module_name in model._modules:
        if len(model._modules[module_name]._modules) > 0:  # 递归至基础模型
            ...
            if hasattr(model._modules[module_name], "num_bit_input"):  # 对基础模型进行赋值
                ...
                #a+=1
                # if a%2==0:
                #     setattr(model._modules[module_name], "quan_bit_input",quan_bit_input*2)
                # else:
                #     setattr(model._modules[module_name], "quan_input", quan_bit_input)
    return model,a
"****************************************************************************************"
