...
class DSQConv_int8_int4(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
                momentum = 0.1,
                num_bit_w=8,num_bit_input=8,QInput = True, bSetQ = True):#bSetQ = True
        super(DSQConv_int8_int4, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
    ...
    def forward(self, x):
        ...
        if self.is_quan:
            ...
            if self.quan_input:
                ...
                delta =  (cur_max - cur_min)/(bit_range_input)#需要改动这里的bit数
                interval = (Qactivation - cur_min) //delta
                mi = (interval + 0.5) * delta + cur_min
                #phi_function就是函数近似操作
                Qactivation = self.phi_function(Qactivation, mi, self.alphaA, delta)
