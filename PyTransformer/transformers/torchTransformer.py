...
class TorchTransformer(nn.Module):
	...
	def __init__(self):
		...
		self._register_dict = OrderedDict()#字典中有DSQconv和DSQLinear
		self.log = Log()		
	...
	def register(self, origin_class, target_class):
		...
		self._register_dict[origin_class] = target_class#原始层改为目标层
	...
	def trans_layers(self, model, update = True):
		...
		if len(self._register_dict) == 0:
			...
			for module_name in model._modules:	#model._modules，就是返回子模块
				# has children 就是还有子模块的情况
				if len(model._modules[module_name]._modules) > 0:
					self.trans_layers(model._modules[module_name], update)
				else:
					if type(getattr(model, module_name)) in self._register_dict:#getattr就是找class中的成员变量（将变量值返回）和成员函数（将函数句柄返回）
						# use inspect.signature to know args and kwargs of __init__
						_sig = inspect.signature(type(getattr(model, module_name)))
						_kwargs = {}
						if update:#复制原moudle的层的参数到新层
							for key in _sig.parameters:
								if _sig.parameters[key].default == inspect.Parameter.empty: #args #应该原始层是没有赋值参数的层
									# assign args
						...
						#https://www.cnblogs.com/starrysky77/p/9102344.html

						#对象的__dict__中存储了一些self.xxx的一些东西
						_attr_dict = getattr(model, module_name).__dict__#获得量化前的层的self.xx的值
						#_register_dict[type(getattr(model, module_name))]得到的是变换后的类名，(**_kwargs)就是类的初始化参数
						_layer_new = self._register_dict[type(getattr(model, module_name))](**_kwargs) # only give positional args
						dict_merge(_layer_new.__dict__, _attr_dict)#将
