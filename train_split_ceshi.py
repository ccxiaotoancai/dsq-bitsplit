...
from DSQConv_split_ceshi1 import DSQConv_split
from DSQLinear import DSQLinear
#from DSQConv1 import DSQConv
...
parser.add_argument('--dist-url', default='tcp://localhost:25559', type=str,
...
parser.add_argument( '--method', default=None, type=str,
                    help='quantization method of nbit.')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.quantize is not None:        
        ...
        if args.quantize == "uniform":		    
            ...
            print("Using  DSQConv_split ...")
            transformer.register(nn.Conv2d, DSQConv_split) #改变注册层的名字
        ...
        if args.quantize_input:
            print("Setting quant method of n_bit : {} ".format(args.method))
            print("Setting quant_split_bit of n_bit : {} ".format(args.quan_bit_input))
            model, _ = set_quanbit_input(model, args.quan_bit_input,0,args.method)
...
def set_quanbit_input(model, quan_bit_input=8,a=0,method="Origin"):  #当a设置为0时其实就是对每一个block的第一个卷积的输出做比特拆分,当a设置为1时就是对每一个block第二个卷积的输出做bit拆分

    for module_name in model._modules:
        if len(model._modules[module_name]._modules) > 0:  # 递归至基础模型

            _,a=set_quanbit_input(model._modules[module_name], quan_bit_input,a,method)
        else:
    ...
    for module_name in model._modules:        
        if len(model._modules[module_name]._modules) > 0:
            ...
            if hasattr(model._modules[module_name], "quan_input"):
                ...
                #print(model._modules[module_name].quan_input)
